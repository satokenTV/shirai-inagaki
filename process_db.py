...
def registration_features(engine, surfaces, bases, poses, adjs, modify_ids, voices, forms, sentence_id):
    ...
    phrase_id = 1
    phrase_id_str = str(sentence_id) + '.' + str(phrase_id)
    for surface, base, pos, adj in zip(surfaces, bases, poses, adjs):
        if surface == '' and base == '' and pos == '' and adj == '':
            phrase_id += 1
            phrase_id_str = str(sentence_id) + '.' + str(phrase_id)
        else:
            session.add(db.Morpheme(surface=surface, base=base, pos=pos, adj=adj, phrase_id=phrase_id_str))
